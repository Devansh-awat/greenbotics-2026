...
# --- Parameters ---
TARGET_RPM = 60.0          # Target wheel RPM
...
MAX_RUN_TIME_SEC = 15.0    # Timeout limit (seconds) in case wall is not detected
...
def run_experiment():
    print("==========================================================")
    print("  GYRO 60 RPM DRIVE + 20CM FRONT DISTANCE STOP EXPERIMENT ")
    print("==========================================================")
    ...
    if not motor.encoder:
        print("ERROR: Encoder not available on motor! Cannot measure speed/stopping time.")
        motor.cleanup()
        return False
    ...
    # Clean initial front distance reading
    ...
    # Start closed-loop wheel RPM control at 60 RPM
    ...
    try:
        while True:
            ...
            # Check Front Distance Trigger (<= 20 cm)
    ...
    if dist_at_stop_signal_cm is not None:
        print(f"Front Distance at Stop Signal: {dist_at_stop_signal_cm:.1f} cm")
    print("=" * 90)

    # High-frequency monitoring of braking / coast-down phase
    print("\nMeasuring stopping duration and coasting distance via encoder...")
    ...
    print(f"Front Dist @ Stop Signal   : {dist_at_stop_signal_cm:.1f} cm" if dist_at_stop_signal_cm is not None else "Front Dist @ Stop Signal   : N/A")
