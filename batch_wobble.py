...
if __name__ == '__main__':
    all_mp4s = sorted(glob.glob('obstacle/*/*.mp4') + glob.glob('open/*/*.mp4'))
    pivot = '2026-04-18_15-06-49'
    ignored_folder = 'obstacle/2026-08-06_23-57-14'

    # Filter out ignored folder
    filtered_mp4s = [f for f in all_mp4s if os.path.dirname(f) != ignored_folder]

    print(f'Starting parallel processing of {len(filtered_mp4s)} video files using {cpu_count()} CPU cores...')

    with Pool(processes=cpu_count()) as pool:
        outputs = pool.map(process_single_video, filtered_mp4s)
    ...
    for f, res, status in outputs:
        ...
        if folder_name >= '2026-08-06_23-20-25':
            category = 'With Wheel Stabilizer (Recent Runs)'
        elif folder_name < pivot:
    ...
    print('\n=== SKIPPED SUMMARY ===')
    for k, v in sorted(skipped_reasons.items(), key=lambda x: x[1], reverse=True)[:10]:
        print(f'{k}: {v}')
    ...
    for name, data in results.items():
        ...
        summary_data[name] = {
            'count': len(data),
            'roll_wobble_rms_mean': float(np.mean(rolls)),
            'roll_wobble_rms_std': float(np.std(rolls)),
            'roll_wobble_max_mean': float(np.mean(roll_maxs)),
            'roll_wobble_max_std': float(np.std(roll_maxs)),
            'vert_bounce_percent_mean': float(np.mean(verts)),
            'vert_bounce_percent_std': float(np.std(verts)),
            'rotational_jerk_mean': float(np.mean(jerks)),
            'rotational_jerk_std': float(np.std(jerks))
        }
        ...
        if name == 'With Wheel Stabilizer (Recent Runs)':
            print('\n  Individual Stabilizer Runs Detail:')
            for item in data:
                p_name = os.path.basename(os.path.dirname(item['path']))
                print(f"    Run {p_name} ({item['duration_s']:.1f}s total): Roll Wobble RMS = {item['roll_wobble_rms_deg']:.4f} deg, Rotational Jerk = {item['rotational_jerk_rms']:.4f} deg/s^2")

    with open('obstacle_wobble_summary.json', 'w') as out_f:
        json.dump(summary_data, out_f, indent=2)

    with open('obstacle_wobble_full.json', 'w') as out_f:
